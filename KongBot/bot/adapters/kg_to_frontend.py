...
class FrontEndNodeAdapter:
    ...
    def get_layout_coords(self, node_id: str):
        ...
        level = 0
        siblings = [node["id"] for node in self.kg.filter_nodes({"depth": level})]
        ...
        while node_id not in siblings:
            if level > self.kg.max_depth():
                ...
                print("Reached max depth and node is not in graph: ", self.kg.get_node(node_id)["node_data"]["title"], "|| ID: ", node_id)
                return 0,0
        ...
        x = level * X_INTERVAL
        # get all nodes
        preceding_nodes = list(self.kg.nodes)[:list(self.kg.nodes).index(node_id)]
