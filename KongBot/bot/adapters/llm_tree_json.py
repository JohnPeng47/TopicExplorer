from bot.base.types import DataNode, NodeType
    ...
    if isinstance(json_data, list):
        ...
        for child in child_nodes:
            for child_key, child_value in child.items():
                child["node_data"]["children"].append(llm_tree_json_adapter(child_key, child_value))
        return child_nodes
    else:
        child_node = DataNode(key, NodeType.TREE_NODE)
        try:            
            for child_key, child_value in json_data.items():
                child_node["node_data"]["children"].append(llm_tree_json_adapter(child_key, child_value))
