...
def test_eval_single_graph(evaluator):
    ...
    # Assert
    assert os.path.exists(evaluator.single_path_csv), "single_path.csv was not generated."
    assert os.path.exists(evaluator.single_path_yaml), "promptfooconfig.yaml was not generated."
    assert os.path.exists(evaluator.prompt_txt), "prompt.txt was not generated."

    # Read and verify CSV
    assert is_csv_files_equal(test_csv_file, evaluator.single_path_csv), "CSV file is not correct."
    assert is_yaml_files_equal(test_yaml_file, evaluator.single_path_yaml), "YAML file is not correct."