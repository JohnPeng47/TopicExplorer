...
from src.KongBot.utils.logger import initialize_logging
...
class KnowledgeGraph(DiGraph):
    ...
    def modify_node(self, node_id: str, data: Dict):
        for key in data.keys():
            self.get_node(node_id)["node_data"][key] = data[key]
    ...
    def _validate_node(self, node: Dict):
        ...
        if not required_keys_node.issubset(set(node.keys())):
            raise Exception(
                "One of these keys are missing: []'id', 'node_data'] ", node)
        # check lvl 2
        else:
            required_keys_node_data = set(["title", "node_type", "children"])
            if not required_keys_node_data.issubset(set(node["node_data"].keys())):
                raise Exception("One of these keys are missing: ", [
                                "title", "node_type", "children"])
    ...
    def display_tree_v2_lineage(self,
        ...
        ancestors = [self.get_node(id) for id in self.ancestors(
        ...
        return ancestors_tree, tree_tree, node["node_data"]["title"]
