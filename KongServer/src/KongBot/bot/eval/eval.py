...
class Eval:
    def __init__(self, output_folder="MY_PROVIDER", num_tests: int = 10):
        ...
        self.output_folder = os.path.join(
            "C:\\Users\\jpeng\\Documents\\business\\kongyiji\\testing\\promptfoo\\examples\\", output_folder)
        self.single_path_csv = os.path.join(
            self.output_folder, "single_path.csv")
        self.single_path_yaml = os.path.join(
            self.output_folder, "promptfooconfig.yaml")
    ...
    def _generate_single_path_csv(self, paths: List[Dict]):
        for path in paths:
            data_fields = [self._get_data_field(
                data).replace("\\n", "") for data in path]
            title_fields = [self._get_title_field(data) for data in path]
            csv_fields = ["TITLE: " + title_fields[i] + ": " +
                          data_fields[i] for i in range(len(path))]
...
"""
    ...
    def gen_diff_graphs(self, lesson_themes: List[str] = None):
        ...
        from src.KongBot.utils import gen_unique_runid
        ...
        curriculums = GenerateCurriculumQuery(
            model="gtp3").get_llm_output() if not lesson_themes else lesson_themes
