...
def generate_sub_trees(graph: KnowledgeGraph):
    ...
    while curr_num_nodes < target_num_nodes:
        if not curr_depth_nodes:
            ...
            curr_depth_nodes = [
                node for node in graph.get_nodes_from_depth(depth)]
        ...
        parent = graph.parents(subtree_id)[0] if graph.parents(
            subtree_id) else subtree_root
        retry = 3
        while retry > 0:
            try:
                subtree_node_new = ascii_tree_to_kg_v2(
                    subtree, subtree_root, parent)
                break
...
def generate_long_description(graph: KnowledgeGraph):
    ...
    nodes_details_query = GenExpandedTextDescription.mt_init(
        cache_policy=cache_policy, model=model)
