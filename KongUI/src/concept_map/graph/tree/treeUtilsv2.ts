...
export class TreeUtils {
  private nodes: Node<RFNodeData>[];
  private edges: Edge<any>[];
  private X_INTERVAL = 50;
  private Y_INTERVAL = 70;

  // Is this consistent with RF nodes??
  ...
  public updateSubtreeJson = (
    ...
    const oldNodes = this.nodes.slice(0, nodeIndex);
    const oldEdges = this.edges.slice(0, nodeIndex - 1);
    ...
    let repoNodes = this.nodes.slice(nodeIndex + numChildrenBefore + 1);
    let repoEdges = this.edges.slice(nodeIndex + numChildrenBefore - 1 + 1);
  ...
  public getNodesBeforeAfter(nodeId: NodeID, numNodes: number)
    ...
    // nodes that came before, not including the parent
    const beforeNodes = this.nodes.slice(0, nodeIndex);
    const beforeEdges = this.edges.slice(0, nodeIndex - 1);

    // nodes that comes after the last children of the parent node
    let afterNodes = this.nodes.slice(nodeIndex + numNodes + 1);
    let afterEdges = this.edges.slice(nodeIndex + numNodes + 1 - 1);
  ...
  //   // nodes that comes after the last children of the parent node
  //   let afterNodes = this.nodes.slice(nodeIndex + 1 + nodeAdded);
  //   let afterEdges = this.edges.slice(nodeIndex - 1 + 1 + nodeAdded);
  ...
  */
  public parent(nodeId: string): Node<RFNodeData> {
    const edgeFromParent = this.edges
